...
/**
 * A modifiable numeric map containing bytes. It doesn't support any new functionality in addition to the
 * functionality of modifiable maps in general and updatable bytes maps.
 *
 ...
 */
public interface ModifiableByteMap<K> extends ModifiableNumericMap<K, Byte>, UpdatableByteMap<K> {
    /**
     * A modifiable numeric map containing bytes and backed by a hash. It implements the
     * {@link net.filipvanlaenen.nombrajkolektoj.bytes.ModifiableByteMap} interface by decorating an
    ...
    public static final class HashMap<K> extends ModifiableByteMapDecorator<K> {
         ...
         */
        private ModifiableHashMap<K, Byte> decoratedMap;
        ...
        public HashMap(final Entry<K, Byte>... entries) {
            decoratedMap = new ModifiableHashMap<K, Byte>(entries);
        }
        ...
        public HashMap(final KeyAndValueCardinality keyAndValueCardinality, final Entry<K, Byte>... entries) {
            decoratedMap = new ModifiableHashMap<K, Byte>(keyAndValueCardinality, entries);
        }
        ...
        public HashMap(final KeyAndValueCardinality keyAndValueCardinality, final Map<? extends K, Byte> source) {
            decoratedMap = new ModifiableHashMap<K, Byte>(keyAndValueCardinality, source);
        }
        ...
        public HashMap(final Map<? extends K, Byte> source) {
            decoratedMap = new ModifiableHashMap<K, Byte>(source);
        }

        @Override
        ModifiableMap<K, Byte> getDecoratedMap() {
            return decoratedMap;
        }
    }
     ...
     */
    static <L> ModifiableByteMap<L> of(final Byte defaultValue, final Collection<? extends L> keys) {
     ...
     */
    static <L> ModifiableByteMap<L> of(final L key, final Byte value) {
     ...
     */
    static <L> ModifiableByteMap<L> of(final L key1, final Byte value1, final L key2, final Byte value2) {
     ...
     */
    static <L> ModifiableByteMap<L> of(final NumericMap<? extends L, Byte> map) {
     ...
     *         the provided bytes maps.
     */
    static <L> ModifiableByteMap<L> unionOf(final KeyAndValueCardinality keyAndValueCardinality,
     ...
     */
    static <L> ModifiableByteMap<L> unionOf(final NumericMap<? extends L, Byte>... maps) {
