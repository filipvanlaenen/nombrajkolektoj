...
/**
 * A modifiable numeric map containing BigDecimals. It doesn't support any new functionality in addition to the
 * functionality of modifiable maps in general and updatable BigDecimals maps.
 *
 ...
 */
public interface ModifiableBigDecimalMap<K> extends ModifiableNumericMap<K, BigDecimal>, UpdatableBigDecimalMap<K> {
    /**
     * A modifiable numeric map containing BigDecimals and backed by a hash. It implements the
     * {@link net.filipvanlaenen.nombrajkolektoj.BigDecimals.ModifiableBigDecimalMap} interface by decorating an
     * {@link net.filipvanlaenen.kolektoj.hash.ModifiableHashMap}.
     *
     * @param <K> The key type.
     */
    public static final class HashMap<K> extends ModifiableBigDecimalMapDecorator<K> {
        /**
         * The internal decorated map.
         */
        private ModifiableHashMap<K, BigDecimal> decoratedMap;
        ...
        public HashMap(final Entry<K, BigDecimal>... entries) {
            decoratedMap = new ModifiableHashMap<K, BigDecimal>(entries);
        }
        ...
        public HashMap(final KeyAndValueCardinality keyAndValueCardinality, final Entry<K, BigDecimal>... entries) {
            decoratedMap = new ModifiableHashMap<K, BigDecimal>(keyAndValueCardinality, entries);
        }
        ...
        public HashMap(final KeyAndValueCardinality keyAndValueCardinality, final Map<? extends K, BigDecimal> source) {
            decoratedMap = new ModifiableHashMap<K, BigDecimal>(keyAndValueCardinality, source);
        }
        ...
        public HashMap(final Map<? extends K, BigDecimal> source) {
            decoratedMap = new ModifiableHashMap<K, BigDecimal>(source);
        }

        @Override
        ModifiableMap<K, BigDecimal> getDecoratedMap() {
            return decoratedMap;
        }
    }
     ...
     */
    static <L> ModifiableBigDecimalMap<L> of(final BigDecimal defaultValue, final Collection<? extends L> keys) {
     ...
     */
    static <L> ModifiableBigDecimalMap<L> of(final L key, final BigDecimal value) {
     ...
     */
    static <L> ModifiableBigDecimalMap<L> of(final L key1, final BigDecimal value1, final L key2, final BigDecimal value2) {
     ...
     */
    static <L> ModifiableBigDecimalMap<L> of(final NumericMap<? extends L, BigDecimal> map) {
        return new HashMap<L>(map);
    }

    /**
     * Returns a new modifiable BigDecimals map with the specified key and value cardinality containing all the entries from
     * the provided BigDecimals maps.
     *
     * @param <L>                    The key type.
     * @param keyAndValueCardinality The key and value cardinality.
     * @param maps                   The BigDecimals maps from which to copy all the entries.
     * @return A new modifiable BigDecimals map with the specified key and value cardinality containing all the entries from
     *         the provided BigDecimals maps.
     */
    static <L> ModifiableBigDecimalMap<L> unionOf(final KeyAndValueCardinality keyAndValueCardinality,
     ...
     */
    static <L> ModifiableBigDecimalMap<L> unionOf(final NumericMap<? extends L, BigDecimal>... maps) {
